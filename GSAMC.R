...
GSAMC=function(x,y, genesets, genenames,
...
censoring.status=NULL,random.seed=NULL,  knn.neighbors=10,
  s0=NULL, s0.perc=NULL,minsize=15,maxsize=500, restand=TRUE,restand.basis=c("catalog","data"), ncores = 1, nperms=200, xl.mode=c("regular","firsttime","next20","lasttime"), xl.time=NULL, xl.prevfit=NULL){
#
...
junk=GSA::GSA.func(x,y,genesets=genesets, genenames=genenames,
...
if(xl.mode=="regular"){
  ...
  result_list <- foreach(i = first:last) %dopar% {
    if (i %% 10 == 0) {
      cat(c("perm=", i, paste("/", as.character(nperms)), sep = ""), fill = TRUE)
    }
    ...
    junk <- GSA::GSA.func(x[, oo], y, genesets = genesets, genenames = genenames, 
    ...
    list(score = junk$score, stand_info = if (restand) unlist(junk$stand.info) else rep(NA, 8))
  }

  stopCluster(cl)

  for (i in 1:length(result_list)) {
    r.star[, i] <- unlist(result_list[[i]]$score)
    stand.info.star[, i] <- unlist(result_list[[i]]$stand_info)
  }
